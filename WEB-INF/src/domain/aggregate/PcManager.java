...
public class PcManager {
	private String[][] pcData = {
			...
			{ "ics866", "133.44.118.223", "true" },
			{ "ics867", "133.44.118.224", "true" },
			{ "ics868", "133.44.118.225", "true" },
			{ "ics869", "133.44.118.226", "true" },
			{ "ics870", "133.44.118.227", "true" },
			{ "ics871", "133.44.118.228", "true" } };

	private List<Pc> pcList = new ArrayList<Pc>();

	private InetAddress fromString(String ipAddress) {
		String[] strIpAddresses = ipAddress.split(".");
		byte[] byteIpAddress=new byte[4];
		for(int j=0;j<4;j++) {
			int ip = Integer.parseInt(strIpAddresses[j]);
			byte byteIp =(byte)ip;
			byteIpAddress[j]=(byte) (byteIp & 0xFF);
		}
	...
	public PcManager() {
		for (int i = 0; i < pcData.length; i++) {
			...
			String strIpAddress = pcData[i][1];
			String[] strIpAddresses = strIpAddress.split(".");
			byte[] byteIpAddress=new byte[4];
			for(int j=0;j<4;j++) {
				int ip = Integer.parseInt(strIpAddresses[j]);
				byte byteIp =(byte)ip;
				byteIpAddress[j]=(byte) (byteIp & 0xFF);
				InetAddress ipAddress = InetAddress.getByAddress(byteIpAddress);
			}
			//byte[] byteIpAddress = (byte[])Arrays.stream(strIpAddresses).map(address -> (byte) (address) & 0xFF).toArray();
